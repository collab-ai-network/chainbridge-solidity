...
contract PhalaBTCLottery {
	...
	modifier onlyNFT() {
    	require(msg.sender == genericHandler, "Permission Denied: Message Sender should be GenericHandler contract");
    	require(tx.origin == nftContract, "Permission Denied: Tx Origin should be NFT contract");
		_;
    }
	...
	constructor(address _nftContract, address _bridge, address _genericHandler) public {
	...
	function isLotteryOpened(uint32 roundId, uint32 tokenId) public view returns (bool)
	{
     ...
     */
	function recordSignedBTCTx(bytes calldata data) external onlyGenericHandler {
	...
	function _newRound(uint32 roundId, uint32 totalCount, uint32 winnerCount) private onlyNFT {
		emit NewRound(roundId, totalCount, winnerCount);
	}

	function _open(uint32 roundId, uint32 tokenId, string memory btcAddress) private {
		require(!openedBox[roundId][tokenId], "Invalid Call: BTC box already opened");

		openedBox[roundId][tokenId] = true;
		emit OpenLottery(roundId, tokenId, btcAddress);
	}

	function _recordSignedBTCTx(uint32 roundId, uint32 tokenId, bytes memory signedTx) private {
